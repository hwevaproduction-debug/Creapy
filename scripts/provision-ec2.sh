...
Provisioning completed. Recommended next steps (manual):
  ...
  3) Do NOT run docker compose from this script. Instead, as the creapy user run:
       sudo -iu creapy /home/creapy/app/scripts/compose-validate.sh
       cd /home/creapy/app
       sudo -iu creapy docker compose -f docker-compose.yml build --pull --parallel
       sudo -iu creapy docker compose -f docker-compose.yml up -d
