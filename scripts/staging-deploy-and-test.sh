...
: ${BACKEND_ROOT:=http://localhost:5000/api/v1}
: ${HEALTH_TIMEOUT:=120}
: ${HEALTH_INTERVAL:=2}
: ${LOG_DIR:=./logs}
...
if ! sh scripts/compose-validate.sh "$COMPOSE_FILE" >/dev/null 2>&1; then
  echo "ERROR: compose validation failed. Helpful commands:" >&2
  echo "  docker compose -f $COMPOSE_FILE config" >&2
  echo "  docker compose -f $COMPOSE_FILE --profile=staging config" >&2
  exit 10
fi

# Step 2: build images
echo "[2/7] Building images (dry run shown; do not run here)..."
# Actual runtime would run: docker compose -f $COMPOSE_FILE build --parallel
# We only write the command for operators to execute on target
if ! printf '%s\n' "docker compose -f $COMPOSE_FILE build --parallel" >/dev/null; then
...
echo "[4/7] Waiting up to ${HEALTH_TIMEOUT}s for backend health at $BACKEND_HEALTH_URL"
count=0
while [ $count -lt $HEALTH_TIMEOUT ]; do
  if curl -sSf "$BACKEND_HEALTH_URL" >/dev/null 2>&1; then
    echo "Backend healthy after ${count}s"
    healthy=1
    break
  fi
...
if [ "${healthy:-0}" != 1 ]; then
  echo "ERROR: backend healthcheck failed after ${HEALTH_TIMEOUT}s" >&2
  echo "Helpful debugging commands:" >&2
  echo "  docker compose -f $COMPOSE_FILE ps" >&2
  echo "  docker compose -f $COMPOSE_FILE logs --no-color --timestamps --tail=200" >&2
...
# docker compose -f $COMPOSE_FILE logs --no-color --timestamps --tail=500 > /tmp/creapy-staging-logs-$(date +%s).log
...
echo "  docker compose -f $COMPOSE_FILE logs --no-color --timestamps --tail=500 > $LOGFILE" >&2
